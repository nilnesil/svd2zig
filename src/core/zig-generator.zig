...
fn concat2(comptime T: type, allocator: Allocator, slice1: []const T, slice2: []const T) anyerror![]T {
    var slice = allocator.alloc(T, slice1.len + slice2.len) catch |err| return err;

    for (slice[0..slice1.len], slice1) |*item, item1| item.* = item1;
    for (slice[slice1.len .. slice1.len + slice2.len], slice2) |*item, item2| item.* = item2;
